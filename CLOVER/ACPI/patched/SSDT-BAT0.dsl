...
DefinitionBlock ("", "SSDT", 2, "hack", "BAT0", 0)
{
    ...
    External(\_SB.PCI0.LPCB.EC.PWUP, MethodObj)
    ...
    {   
        ...
        Method (BTIF, 1, Serialized)
        {
                ...
                \_SB.PCI0.LPCB.EC.BTDR (One)
                If (LEqual (\_SB.PCI0.LPCB.EC.BSTA (Local7), 0x0F))
                {
                    Return (0xFF)
                }

                Acquire (\_SB.PCI0.LPCB.EC.BTMX, 0xFFFF)
                Store (\_SB.PCI0.LPCB.EC.NGBF, Local0)
                Release (\_SB.PCI0.LPCB.EC.BTMX)
                ...
                Acquire (\_SB.PCI0.LPCB.EC.BTMX, 0xFFFF)
                Or (\_SB.PCI0.LPCB.EC.NGBT, Local7, \_SB.PCI0.LPCB.EC.NGBT)
                Release (\_SB.PCI0.LPCB.EC.BTMX)
                If (\_SB.PCI0.LPCB.EC.ECRG)
                {
                    ...
                    Multiply (Local0, \_SB.PCI0.LPCB.EC.NLB1, Local1)
                    Divide (Add (Local1, 0x63), 0x64, Local3, Local2)
                    Store (Local2, Index (DerefOf (Index (\_SB.NBTI, Arg0)), 0x05))
                    Store (\_SB.PCI0.LPCB.EC.NLB1, Index (DerefOf (Index (\_SB.NBTI, Arg0)), 0x05))
                    Store (\_SB.PCI0.LPCB.EC.NLB2, Index (DerefOf (Index (\_SB.NBTI, Arg0)), 0x06))
                    While (One)
                    {
                        Store (Arg0, _T_1)
                        If (LEqual (_T_1, Zero))
                ...
                Store (\_SB.PCI0.LPCB.EC.GBSS (Local0, Local1), Local2)
                Store (Local2, Index (DerefOf (Index (\_SB.NBTI, Arg0)), 0x0A))
                Acquire (\_SB.PCI0.LPCB.EC.BTMX, 0xFFFF)
                And (\_SB.PCI0.LPCB.EC.NGBF, Not (Local7), \_SB.PCI0.LPCB.EC.NGBF)
                Release (\_SB.PCI0.LPCB.EC.BTMX)
                Return (Zero)
        }
        ...
        Method (ITLB, 0, NotSerialized)
        {
            If (\_SB.PCI0.LPCB.EC.ECRG)
            {
                ...
                Multiply (Local3, ^^LB1, \_SB.PCI0.LPCB.EC.NLB1)
                Multiply (Local3, ^^LB2, \_SB.PCI0.LPCB.EC.NLB2)
            }
        }
        ...
        Method (GBTI, 1, NotSerialized)
        {
                If (\_SB.PCI0.LPCB.EC.ECRG)
                {
                    If (And (^^BTAP, ShiftLeft (One, Arg0)))
                    {
                        ...
                        Store (\_SB.PCI0.LPCB.EC.GBSS (B1B2 (^^SN00, ^^SN01), B1B2 (^^MD00, ^^MD01)), BTSN)
